...
#       - $AUTH_DOMAIN: Authentication domain, where the DNS challenge will take place (i.e. TXT records are created here).
#       - $CERT_EMAIL: The email address to be included in the certificate.
# - The following environment variables SHOULD be provided:
#       - $DEPLOY_HOOK: The deploy-hook command.  In most cases, this should be a script which can deploy the generated
...
# - Files generated by this image should be saved and re-loaded between runs.  There are two ways this can be done:
#        1. Mounting a volume (or some other form of persistent storage) to $CERT_WORKING_DIR.  The location
#           of $CERT_WORKING_DIR can be found in the first ENV statement of this Dockerfile.
#        2. Providing both the $SAVE_HOOK and $LOAD_HOOK commands.  Both hooks MUST be provided in order for
#           this image to work correctly.  Both hooks will be provided 1 argument:
#              - $SAVE_HOOK: the path of the folder whose content needs to be saved.
#              - $LOAD_HOOK: the path of the folder that the content from previous runs needs to be loaded into.
